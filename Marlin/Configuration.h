 ...
 */
  ...
  #define MPC_BLOCK_HEAT_CAPACITY { 20.40 }           // (J/K) Heat block heat capacities.
  #define MPC_SENSOR_RESPONSIVENESS { 0.1433 }         // (K/s per ∆K) Rate of change of sensor temperature from heat block.
  #define MPC_AMBIENT_XFER_COEFF { 0.0968 }           // (W/K) Heat transfer coefficients from heat block to room air with fan off.
  #if ENABLED(MPC_INCLUDE_FAN)
    #define MPC_AMBIENT_XFER_COEFF_FAN255 { 0.1084 }  // (W/K) Heat transfer coefficients from heat block to room air with fan on full.
  #endif
